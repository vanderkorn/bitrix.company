			...
			<td width="<?=$arResult["TD_WIDTH"]?>" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
				...
				<tr>
					...
					<?if(is_array($arElement["PREVIEW_PICTURE"])):?>
						<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img border="0" src="<?=$arElement["PREVIEW_PICTURE"]["SRC"]?>" width="<?=$arElement["PREVIEW_PICTURE"]["WIDTH"]?>" height="<?=$arElement["PREVIEW_PICTURE"]["WIDTH"]?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a>
					<?endif?>
					</td>
					<td valign="top">
...
<?=PathSections($arElement["IBLOCK_SECTION_ID"])?>

<br />
						<?foreach($arElement["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
						<?if($arProperty["CODE"] =="vdk_city"):?>

						<?=$arProperty["NAME"]?>:&nbsp;<?=strip_tags($arProperty["DISPLAY_VALUE"]);?><br /> 

						<?elseif ($arProperty["CODE"] =="vdk_email"):?>

						<?=$arProperty["NAME"]?>:&nbsp;<a href="mailto:<?=$arProperty["DISPLAY_VALUE"];?>" target="_blank"><?=$arProperty["DISPLAY_VALUE"];?></a><br /> 
						
						<?elseif ($arProperty["CODE"] =="vdk_user"):?>
						<?
						$ID_USER = $arProperty["DISPLAY_VALUE"];
						$rsUser = CUser::GetByID($ID_USER);
						$arUser = $rsUser->Fetch();
						$NAME_USER = $arUser['LAST_NAME']." ".$arUser['NAME']; 
						?>
						<?=$arProperty["NAME"]?>:&nbsp;<a href="/club/user/<?=$ID_USER;?>/" target="_blank"><?=$NAME_USER;?></a><br /> 
 						<?elseif ($arProperty["CODE"] =="vdk_specialist" && $arProperty["VALUE_XML_ID"]=="vdk_specialist_no"):?>
...
<a href="<?=$arElement["DETAIL_PAGE_URL"]?>">Подробное описание</a><br />
		...
		<?if($arResult["bDisplayPrices"]):?>
			...
			<?if(is_array($arElement)):?>
				<?if(is_array($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):?>
					<td width="<?=$arResult["TD_WIDTH"]?>" class="data-cell" rowspan="2">
